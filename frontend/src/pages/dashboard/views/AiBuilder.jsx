...
import { useLocation } from "react-router-dom"; // 🛠️ Added useLocation
...
export default function AIBuilder() {
  const location = useLocation(); // 🛠️ Initialize location instance
  const [brief, setBrief] = useState("");
  ...
  // 🛠️ Check for incoming template data on tab redirect
  useEffect(() => {
    if (location.state?.prefilledTemplate) {
      const template = location.state.prefilledTemplate;
      
      // Inject template ready-made text content directly into your brief textbox state!
      setBrief(template.content);
      
      // Clear out the browser history entry context so normal refreshes start clean
  ...
  const handleGenerateDraft = async () => {
    try {
      setLoading(true);
      setError("");
      const response = await aiApi.generateDraft({ brief }, getToken());
      setDraft(response);
    } catch (error) {
      setError(error.message);
    } finally {
  ...
  const handleImproveScore = async () => {
    try {
      setLoading(true);
      setError("");
      const response = await aiApi.improveWinScore(
        { brief, currentScore: draft?.score || 0 },
        getToken()
      );
      setImprovedScore(response);
  ...
  const handleApplySuggestion = async () => {
    try {
      setLoading(true);
      setError("");
      const response = await aiApi.applySuggestion({ brief, suggestion }, getToken());
      setDraft(response);
    } catch (error) {
      setError(error.message);
    } finally {
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div>
        <h1 className="text-3xl font-bold text-white">AI Proposal Studio</h1>
        <p className="text-slate-400 mt-2">Generate winning proposals using AI.</p>
      </div>
      ...
      {draft && (
        <div className="rounded-2xl border border-emerald-900/20 bg-slate-900 p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-white">Win Score</h2>
              <p className="text-slate-400">Improve proposal quality using AI</p>
            </div>
            <button
      ...
      {suggestion && (
        <div className="rounded-2xl border border-emerald-900/20 bg-slate-900 p-6">
          ...
          <p className="text-slate-300 mb-4">{suggestion}</p>
          <button
      ...
      {improvedScore && (
        <div className="rounded-2xl border border-emerald-900/20 bg-slate-900 p-6">
          <h2 className="text-xl font-semibold text-white mb-2">Improved Score</h2>
          <pre className="overflow-auto rounded-xl bg-slate-950 p-4 text-slate-300 whitespace-pre-wrap">
            {JSON.stringify(improvedScore, null, 2)}
          </pre>
        </div>
      )}
    </div>
