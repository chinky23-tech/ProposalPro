...
import { ArrowLeft, Send, Copy, Check, Link2, ExternalLink, Mail } from "lucide-react";
...
export default function ProposalPreview() {
  ...
  // 🛠️ State to hold generated link info for the UI
  ...
  const handleShareWithClient = async () => {
    ...
    try {
      ...
      if (url) {
        ...
        // Update local proposal status to 'Sent' instantly
  ...
  return (
    <div className="space-y-6 p-6 max-w-4xl mx-auto text-slate-100">
      ...
      {/* 🚀 PERMANENT SHARE HUD CARD (Renders when shareData is populated) *
      {shareData && (
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-center gap-2 px-3 h-9 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs text-slate-300 border border-slate-700 transition-colors"
                title="Test Client View"
              >
...
export default function ProposalPreview() {
  ...
  // Share details & Modal visibility state
  const [shareData, setShareData] = useState(null); 
  ...
  const handleShareWithClient = async () => {
    ...
    try {
      ...
      if (url) {
        ...
        setProposal((prev) => ({ ...prev, status: "Sent" }));
        setIsShareModalOpen(true); // 🚀 Open Social Share Modal Automatically!
        toast.success("Proposal link created and email dispatched!");
      } else {
        toast.warning("Email sent, but could not extract the share link for preview.");
      }
  ...
  // Helper text template for social messaging
  const shareMessage = `Hi! Here is the digital proposal for "${proposal?.title || "our project"}": ${shareData?.url}`;

  // 🚀 Native Web Share API (Triggers native iOS/Android/macOS share sheet)
    ...
    <div className="space-y-6 p-6 max-w-4xl mx-auto text-slate-100">
      ...
      {isShareModalOpen && shareData && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-lg w-full space-y-6 shadow-2xl animate-fadeIn">
